...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    '''
    ignore = hashtag_to_ignore.lower()[1:] #remove # symbol from querry
    list_of_tags = []
    ...
    max_count = 0
    most_popular_tag = ""
    #loop over the dictionary of key,val pairs. Return the key with the highest count/value
    #if there is a tie, it only returns the first instance with the highest count
    for key,val in counts_of_hastags.items():
        if val > max_count:
            most_popular_tag = key
            max_count = val
